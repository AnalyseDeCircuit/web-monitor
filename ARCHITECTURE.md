...
graph LR
    ...
    subgraph Server["Go Server"]
        ...
        subgraph Collection["Streaming Aggregator"]
            direction TB
            FastCollectors["Fast Collectors<br/>(CPU/Mem/Net)<br/>User Interval"]
            SlowCollectors["Slow Collectors<br/>(Disk/GPU/SSH)<br/>2-10s"]
            AtomicStorage["Atomic Storage<br/>Per-Module"]
        end
    ...
    FastCollectors --> AtomicStorage
    SlowCollectors --> AtomicStorage
    FastCollectors --> Host
...
### Data Flow with Streaming Aggregator
...
flowchart TD
    ...
    subgraph StreamingAggregator["Streaming Aggregator"]
        AtomicStore["Atomic Storage<br/>(Per-Module)"]
        MergeFunc["GetLatestStats()<br/>Instant Merge"]
    end
    
    subgraph FastCollectors["Fast Collectors (2-60s)"]
        C1[CPU]
        C2[Memory]
        C3[Network]
    end
    
    subgraph SlowCollectors["Slow Collectors"]
        C4[Disk<br/>2s]
        C5[GPU<br/>2s]
        C6[Sensors<br/>2s]
        C7[Power<br/>3s]
    ...
    M -->|Check Cache| Cache
    Cache -->|Hit: Return| M
    Cache -->|Miss: Fetch| MergeFunc
    
    WS --> H
    H -->|Broadcast| MergeFunc
    
    Cfg -->|Enable Flags| FastCollectors
    Cfg -->|Enable Flags| SlowCollectors
    
    FastCollectors -->|Independent Goroutines| AtomicStore
    SlowCollectors -->|Independent Goroutines| AtomicStore
    AtomicStore --> MergeFunc
    
    MergeFunc -->|Check Thresholds| Alert[Alert Manager]
```
...
sequenceDiagram
    ...
    participant Fast as Fast Collectors
    participant Slow as Slow Collectors
    participant Store as Atomic Storage

    Note over Agg,Store: Collectors run independently
    
    par Independent Collection
        Fast->>Store: CPU/Mem/Net (user interval)
        Slow->>Store: Disk/GPU (2s)
        Slow->>Store: Power (3s)
        Slow->>Store: SSH (5s)
        Slow->>Store: System (10s)
    end
    
    C1->>Hub: Connect (interval: 5s)
    Hub->>Agg: Start Broadcasting
    
    loop Every 5 seconds
        Agg->>Store: GetLatestStats()
        Store-->>Agg: Instant Merge (no blocking)
        Agg->>Hub: Merged Snapshot
        Hub->>C1: Broadcast JSON
    end
    
    C2->>Hub: Connect (interval: 3s)
    Hub->>Hub: Update Interval (min: 3s)
    
    loop Every 3 seconds
        Agg->>Store: GetLatestStats()
        Store-->>Agg: Instant Merge
        Hub->>C1: Broadcast
        Hub->>C2: Broadcast
    end
    
    C1->>Hub: Disconnect
    Hub->>Hub: Recalculate Interval
```
...
graph TB
    ...
    subgraph StreamingAggregator["Streaming Aggregator"]
        Start["Start()"]
        AtomicStore["Atomic Storage<br/>(Per-Module Data)"]
        GetStats["GetLatestStats()<br/>Instant Merge"]
    end
    
    subgraph FastCollectors["Fast Collectors (User Interval: 2-60s)"]
    ...
    subgraph SlowCollectors["Slow Collectors (Fixed Intervals)"]
        subgraph Medium["2s Interval"]
        ...
        subgraph Slower["3-10s Interval"]
            Power["Power Collector<br/>- Battery & AC<br/>- Power Profile<br/>(3s)"]
            SSH["SSH Collector<br/>- Active Sessions<br/>- Auth Methods<br/>(5s)"]
            Sys["System Collector<br/>- Boot Time<br/>- Process List<br/>(10s)"]
        end
    end
    
    Flags -->|"Enable/Disable"| Start
    Start -->|"Spawn Goroutines"| FastCollectors
    Start -->|"Spawn Goroutines"| SlowCollectors
    
    CPU -->|"atomic.Store"| AtomicStore
    Mem -->|"atomic.Store"| AtomicStore
    Net -->|"atomic.Store"| AtomicStore
    Disk -->|"atomic.Store"| AtomicStore
    GPU -->|"atomic.Store"| AtomicStore
    Sensors -->|"atomic.Store"| AtomicStore
    Power -->|"atomic.Store"| AtomicStore
    SSH -->|"atomic.Store"| AtomicStore
    Sys -->|"atomic.Store"| AtomicStore
    
    AtomicStore -->|"atomic.Load"| GetStats
    GetStats -->|"CPU/Mem/Disk %"| Alert[Alert Manager]
    GetStats -->|"JSON Response"| WSHub[WebSocket Hub]
```
...
- **Streaming Aggregator**: Uses pre-collected data from atomic storage
- **Broadcast Mechanism**: Sends metric updates to all connected clients at the minimum requested interval
- **Non-blocking**: Data retrieval is instant (atomic.Load), no goroutine blocking

#### 5.4 Streaming Aggregator (`internal/collectors/aggregator`)
- **Independent Goroutines**: Each collector runs in its own goroutine with its own interval
- **Atomic Storage**: Each module stores data via `atomic.Value` for thread-safe access
- **Instant Merge**: `GetLatestStats()` merges all module data without blocking
- **Interval Tiers**:
  - Fast (CPU/Memory/Network): User-selected interval (2-60s)
  - Medium (Disk/Sensors/GPU): 2 seconds
  - Slow (Power): 3 seconds
  - Slower (SSH): 5 seconds
  - Slowest (System): 10 seconds
- **Benefits**:
  - Fast metrics not blocked by slow collectors
  - Stable memory usage (no goroutine pile-up)
  - Responsive UI even with slow SSH/System collectors
...
**Streaming Aggregator Architecture**: Each collector runs in an independent goroutine with its own update interval. Data is stored atomically and merged instantly on request.

**Fast Collectors** (User-selected interval: 2-60s):
...
3. **Network Collector**: Interface stats, traffic counters (bytes sent/recv)

**Medium Collectors** (2s interval):
4. **Disk Collector**: Partition info, usage, filesystem types, IO stats, inodes
...
**Slow Collectors** (3-10s intervals):
7. **Power Collector** (3s): Battery status, AC adapter, power profiles
8. **SSH Collector** (5s): Active sessions, authentication methods, OOM risk processes
9. **System Collector** (10s): Boot time, process list

All collectors use `gopsutil` and respect HostFS environment variables for container deployments.

**Modularity**: Each collector can be disabled via environment variable. Disabled collectors:
...
- **Canvas Charts**: HiDPI-aware Canvas rendering for sparklines and pie charts (vs SVG)
- **DOM Reuse**: `updateList()` utility for efficient list updates without recreating elements
- **Render Throttling**: 10 FPS max render rate for WebSocket data
- **CSS `will-change`**: Hardware-accelerated progress bar animations
