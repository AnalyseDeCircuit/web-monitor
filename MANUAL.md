...
### 最新版本 (v1.4)
*   **架构重构**: 引入新的 `system` 模块，负责系统信息采集；`gpu` 模块增强对 NVIDIA/AMD/Intel GPU 的监控
*   **功能增强**: 进程管理功能改进，支持更多排序选项；系统信息展示更全面
*   **容器兼容性优化**: 改进 `/hostfs` 挂载机制，提升在容器环境中执行宿主机命令的可靠性
*   **文档更新**: 完善使用手册和故障排除指南
