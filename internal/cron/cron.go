...
func ListCronJobs() ([]types.CronJob, error) {
	...
	// 获取用户crontab
	cmd = exec.Command("bash", "-c", "for user in $(cut -f1 -d: /etc/passwd); do crontab -u $user -l 2>/dev/null || true; done")
	output, err = cmd.Output()
	if err == nil {
		scanner := bufio.NewScanner(bytes.NewReader(output))
		currentUser := ""
		...
		for scanner.Scan() {
			...
			if strings.HasPrefix(line, "# DO NOT EDIT THIS FILE") {
				// 这是crontab文件头，下一行是用户
				if scanner.Scan() {
					userLine := scanner.Text()
					if strings.HasPrefix(userLine, "# (") && strings.HasSuffix(userLine, ")") {
						currentUser = strings.TrimPrefix(strings.TrimSuffix(userLine, ")"), "# (")
					}
				}
				continue
			}
